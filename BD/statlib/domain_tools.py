...
#-------------------------------
# Spark preprocessing functions
#-------------------------------


def create_spark_session(
    app_name: str = "FlightPreprocessSpark",
    master_url: str = "spark://10.0.0.79:7077",
...
def build_df_final_clean_from_airports_raw_spark(
    flights_raw,
    airports_raw,
):
    """
    Build the cleaned flight dataframe using PySpark transformations.

    Returns:
        tuple[DataFrame, DataFrame, DataFrame]:
            df_final_clean, neo_airports, neo_flights
    """
    import pandas as pd
    ...
    def is_spark_df(obj) -> bool:
        return hasattr(obj, "sparkSession") and hasattr(obj, "withColumn") and hasattr(obj, "columns")

    flights_is_spark = is_spark_df(flights_raw)
    airports_is_spark = is_spark_df(airports_raw)
    flights_is_pandas = isinstance(flights_raw, pd.DataFrame)
    airports_is_pandas = isinstance(airports_raw, pd.DataFrame)

    # Spark Dataframe used below
    if flights_is_spark and airports_is_spark:
        flights_df = flights_raw
        airports_df = airports_raw
        print("DataFrames are already PySpark DataFrames. Proceeding with Spark transformations.")
    elif flights_is_pandas and airports_is_pandas:
        spark = create_spark_session()
        spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
        spark.conf.set("spark.sql.execution.arrow.pyspark.fallback.enabled", "false")
        flights_df = spark.createDataFrame(flights_raw)
        airports_df = spark.createDataFrame(airports_raw)
        print("Converted pandas DataFrames to PySpark DataFrames. Proceeding with Spark transformations.")
    else:
        raise TypeError(
            "Inputs must be both PySpark DataFrames or both pandas DataFrames."
        )

    AIRLINE_COL: str = pick_col(flights_df.columns, ["AIRLINE", "AIRLINES_IATA_CODE"])
    ORIGIN_COL: str = pick_col(flights_df.columns, ["ORIGIN_AIRPORT_IATA_CODE", "ORIGIN_AIRPORT"])
    DEST_COL: str = pick_col(flights_df.columns, ["DESTINATION_AIRPORT_IATA_CODE", "DESTINATION_AIRPORT"])

    # Build DATETIME only when all date parts are available.
    date_parts = ["YEAR", "MONTH", "DAY"]
    if all(c in flights_df.columns for c in date_parts):
    ...
    # HHMM -> native Timestamp conversion
    for c in ["SCHEDULED_DEPARTURE", "DEPARTURE_TIME", "SCHEDULED_ARRIVAL", "ARRIVAL_TIME"]:
        if c in flights_df.columns:
            flights_df = flights_df.withColumn(c, format_hhmm_pd_spark(c))

    # ========================================================================
    # 1. NORMALIZATION OF COLUMNS AND IATA CODES (withColumn)
    # Lazy transformation: the Driver adds these steps to the execution plan (DAG),
    # but no computation is actually executed on the Executors yet.
    # ========================================================================
    flights_df = (
        flights_df
        .withColumn(ORIGIN_COL, F.upper(F.trim(F.col(ORIGIN_COL).cast("string"))))
        .withColumn(DEST_COL, F.upper(F.trim(F.col(DEST_COL).cast("string"))))
    )

    airport_code_col = pick_col(airports_df.columns, ["AIRPORTS_IATA_CODE", "IATA_CODE"])

    origin_airports = (
        airports_df
        .withColumnRenamed(airport_code_col, "ORIGIN_AIRPORTS_IATA_CODE")
        ...
        .withColumn("ORIGIN_AIRPORTS_IATA_CODE", F.upper(F.trim(F.col("ORIGIN_AIRPORTS_IATA_CODE").cast("string"))))
    )

    dest_airports = (
        airports_df
        .withColumnRenamed(airport_code_col, "DEST_AIRPORTS_IATA_CODE")
        ...
        .withColumn("DEST_AIRPORTS_IATA_CODE", F.upper(F.trim(F.col("DEST_AIRPORTS_IATA_CODE").cast("string"))))
    )

    # ========================================================================
    # 2. JOIN WITH ORIGIN/DESTINATION AIRPORTS
    # Shuffle operation: this is the most expensive operation in Spark. Data is 
    # redistributed among Executors in parallel to align the join keys.
    # ========================================================================
    df = (
        flights_df
        .join(origin_airports, flights_df[ORIGIN_COL] == origin_airports["ORIGIN_AIRPORTS_IATA_CODE"], "left")
        .join(dest_airports, flights_df[DEST_COL] == dest_airports["DEST_AIRPORTS_IATA_CODE"], "left")
    )

    arr_pos = F.greatest(F.coalesce(F.col("ARRIVAL_DELAY").cast("double"), F.lit(0.0)), F.lit(0.0))
    dep_pos = F.greatest(F.coalesce(F.col("DEPARTURE_DELAY").cast("double"), F.lit(0.0)), F.lit(0.0))
    cancel_is_empty = F.col("CANCELLATION_REASON").isNull() | (F.trim(F.col("CANCELLATION_REASON")) == "")

    # ========================================================================
    # 3. TOTAL_DELAY CALCULATION WITH COLUMNAR FUNCTIONS
    # Spark uses optimized columnar expressions (unlike Pandas, which is 
    # oriented towards rows/Python objects), applying the logic in a vectorized 
    # manner across partitions during execution.
    # ========================================================================
    df = df.withColumn(
        "TOTAL_DELAY",
        F.when(cancel_is_empty, (arr_pos + dep_pos) / F.lit(2.0)).otherwise(F.lit(None).cast("double")),
    )

    max_delay = df.agg(F.max("TOTAL_DELAY").alias("max_delay")).first()[0]
    max_delay_lit = F.lit(float(max_delay) if max_delay is not None else 0.0)

    diverted_col = F.col("DIVERTED").cast("boolean") if "DIVERTED" in df.columns else F.lit(False)
    df = df.withColumn(
        "TOTAL_DELAY",
        F.when(diverted_col, max_delay_lit)
        .when(~cancel_is_empty, max_delay_lit)
        .otherwise(F.col("TOTAL_DELAY")),
    )
    ...
    neo_fields = [c for c in neo_fields if c in df.columns]
    df_base_final = df.select(*neo_fields)

    if "CANCELLATION_REASON" in df_base_final.columns:
        reason_map = F.create_map(*[F.lit(x) for kv in BTS_MAP_CANCELLATION_REASON.items() for x in kv])
        df_base_final = df_base_final.withColumn(
            "CANCELLATION_REASON",
            F.coalesce(reason_map[F.col("CANCELLATION_REASON")], F.col("CANCELLATION_REASON")),
        )

    nullable_cols = {"CANCELLATION_REASON"}
    not_nullable_cols = [c for c in df_base_final.columns if c not in nullable_cols]
    df_final_clean = df_base_final.dropna(subset=not_nullable_cols)

    id_window = Window.orderBy(F.monotonically_increasing_id())
    df_final_clean = df_final_clean.withColumn("FLIGHT_ID", F.row_number().over(id_window))

    # ========================================================================
    # 4. FINAL CLEANUP AND CONSTRUCTION OF neo_airports AND neo_flights TABLES
    # unionByName and dropDuplicates involve further shuffle/aggregation operations 
    # to guarantee the uniqueness of airport nodes at a distributed level.
    # ========================================================================
    origin_nodes = df_final_clean.select(
        F.col("ORIGIN_AIRPORTS_IATA_CODE").alias("IATA"),
    ...
    dest_nodes = df_final_clean.select(
        F.col("DEST_AIRPORTS_IATA_CODE").alias("IATA"),
    ...
    neo_airports = origin_nodes.unionByName(dest_nodes).dropDuplicates(["IATA"])

    neo_flights = df_final_clean.select(
        F.col("FLIGHT_ID"),
        F.col("DATETIME"),
        F.col(AIRLINE_COL).alias("AIRLINE_CODE"),
        F.col("ORIGIN_AIRPORTS_IATA_CODE").alias("ORIGIN_IATA"),
        F.col("DEST_AIRPORTS_IATA_CODE").alias("DEST_IATA"),
        F.col("DISTANCE"),
        F.col("TOTAL_DELAY"),
    ...
    # ========================================================================
    # 5. FINAL MATERIALIZATION TO PANDAS (.toPandas())
    # This is an ACTION. Only now does Spark actually execute the DAG planned 
    # by the Driver. The Executors process the data in parallel, and the result 
    # is collected into the Driver's memory as a Pandas DataFrame.
    # Note: Spark has an initial overhead, but on big data it outperforms Pandas, 
    # which would work in-memory on a single node in eager mode.
    # ========================================================================
    df_final_clean = df_final_clean.toPandas()
    neo_airports = neo_airports.toPandas()
    neo_flights = neo_flights.toPandas()
...
def create_sail_session(
    ...
    return (
        SparkSession.builder
        .appName(app_name)
        .remote("sc://10.0.0.79:7077")
        .config("spark.driver.bindAddress", driver_bind_address)
        .getOrCreate()
    )


def build_df_final_clean_from_airports_raw_sail(
    flights_raw,
    airports_raw,
):
    """
    ...
    def normalize_iata(col_name: str):
        return F.upper(F.trim(F.col(col_name).cast("string")))

    def hhmm_to_time_string(col_name: str):
        raw = F.col(col_name).cast("int")
        normalized = F.when(raw == 2400, F.lit(0)).otherwise(raw)
        hh = F.floor(normalized / 100)
        mm = normalized % 100
    ...
    if flights_is_spark and airports_is_spark:
        ...
        # Force Arrow path to avoid PythonRDD execution on workers.
        spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
        spark.conf.set("spark.sql.execution.arrow.pyspark.fallback.enabled", "false")
        flights_df = spark.createDataFrame(flights_raw)
        airports_df = spark.createDataFrame(airports_raw)
    ...
    AIRLINE_COL = pick_col(flights_df.columns, ["AIRLINE", "AIRLINES_IATA_CODE"])
    ORIGIN_COL = pick_col(flights_df.columns, ["ORIGIN_AIRPORT_IATA_CODE", "ORIGIN_AIRPORT"])
    DEST_COL = pick_col(flights_df.columns, ["DESTINATION_AIRPORT_IATA_CODE", "DESTINATION_AIRPORT"])
    ...
    for c in ["SCHEDULED_DEPARTURE", "DEPARTURE_TIME", "SCHEDULED_ARRIVAL", "ARRIVAL_TIME"]:
        if c in flights_df.columns:
            flights_df = flights_df.withColumn(c, hhmm_to_time_string(c))

    flights_df = (
        flights_df
        .withColumn(ORIGIN_COL, normalize_iata(ORIGIN_COL))
        .withColumn(DEST_COL, normalize_iata(DEST_COL))
    )
    ...
    origin_airports = (
        ...
        .withColumnRenamed("LATITUDE", "ORIGIN_LATITUDE")
        .withColumnRenamed("LONGITUDE", "ORIGIN_LONGITUDE")
        .withColumn("ORIGIN_AIRPORTS_IATA_CODE", normalize_iata("ORIGIN_AIRPORTS_IATA_CODE"))
    )
    ...
    dest_airports = (
        ...
        .withColumnRenamed("LATITUDE", "DEST_LATITUDE")
        .withColumnRenamed("LONGITUDE", "DEST_LONGITUDE")
        .withColumn("DEST_AIRPORTS_IATA_CODE", normalize_iata("DEST_AIRPORTS_IATA_CODE"))
    )
